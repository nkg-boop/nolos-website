...
- Explicit security headers (CSP, HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy) are set in `vercel.json` for every route. Vercel's platform applies HSTS automatically on its own, per Vercel's documentation, but does not automatically add the others — these need explicit configuration, which is what `vercel.json` does here.
